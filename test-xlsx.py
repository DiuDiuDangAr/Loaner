...
import smtplib
import pandas as pd
from send_email_xlsx import send_email_xlsx
from read_loaning_xlsx import read_loaning_xlsx
from cut_into_dict import cut_into_dict
from read_sales_csv import read_sales_csv
from check_due_date_xlsx import check_due_date_xlsx

# 1. read loaning csv:
#		input: csv
#		output: raw data numpy array
sorted_due_instr_list = read_loaning_xlsx()

# 2-1. check due day
#		input: raw data numpy array
#		output: overtimed sorted numpy array, about to due within 7 days 
sorted_due_instr_list = check_due_date_xlsx(sorted_due_instr_list)
...
for index, row in sorted_sales_list.iterrows(): 	
	...
	try:
		...
		continue
	#send_email_xlsx('welly.huang@ni.com',[row['sales_email'],row['sales_manager_email1'],'welly.huang@ni.com'],title,context)
	send_email_xlsx('welly.huang@ni.com','welly.huang@ni.com',title,context)
